 ...
 *
 * 用法：node scripts/build-plugin-store.cjs [<仓库根目录>] [-o <输出目录>]
...
function collectFiles(dir, rel = '', out = []) {
  for (const entry of fs.readdirSync(path.join(dir, rel), { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) {
      ...
      if (name.endsWith('.mixplugin') || name === 'store.json') continue
...
function readManifest(pluginDir, dirName) {
...
function main() {
  ...
  for (const entry of fs.readdirSync(root, { withFileTypes: true })) {
    ...
    plugins.push({
      ...
      path: entry.name,
      package: zipFile,
      package_md5: md5(buffer),
      ...meta
    })
    console.log(`[ok] ${manifest.name} v${manifest.version} -> ${zipPath} (${buffer.length} bytes)`)
  }
